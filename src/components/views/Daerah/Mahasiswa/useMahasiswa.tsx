...
const useMahasiswa = () => {
  ...
  const getMumi = async () => {
    ...
    const res = await generusServices.getGenerusByMahasiswaDaerah(idDaerah, params);
    const { data } = res;
    return data;
  };
  ...
  const {
    ...
    queryFn: getMumi,
    enabled: !!idDaerah && isParamsReady && !!currentPage && !!currentLimit,
  });
